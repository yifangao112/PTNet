...
from network_architecture.custom.config.type_definitions import NdarrayOrTensor, PathLike
from network_architecture.custom.utils.module import version_leq
...
def set_determinism(
    ...
    Note:
        ...
        This function will not affect the randomizable objects in :py:class:`transforms.Randomizable`, which
        have independent random states. For those objects, the ``set_random_state()`` method should be used to
        ensure the deterministic behavior (alternatively, :py:class:`data.DataLoader` by default sets the seeds
        according to the global random state, please see also: :py:class:`data.utils.worker_init_fn` and
        :py:class:`data.utils.set_rnd`).
    """
    if seed is None:
